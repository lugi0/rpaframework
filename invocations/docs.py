...
def build_python_markdown(ctx):
    ...
    # shell.sphinx(ctx, f"-M clean {docs_source} {docs_target}")
    shell.sphinx(ctx, f"-M markdown {docs_source} {docs_target}")
    ...
    libraries = []
    for library_path in (DOCS_BUILD_DIR / "markdown" / "libraries").iterdir():
        if not library_path.is_dir():
            continue
    ...
    output = DOCS_ROOT / "build" / "markdown.json"
